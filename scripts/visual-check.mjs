...
// 육안 확인은 본질적으로 사람 몫이라 자동 판정은 못 하지만, "확인했다"는 명시적 서명을
// 요구해 pre-commit 훅이 강제한다(honor-system 아님).
//
// 기록:  node scripts/visual-check.mjs <feature> --pass --by <name>
//        node scripts/visual-check.mjs <feature> --fail --by <name> [--note "..."]
// 검증:  node scripts/visual-check.mjs <feature>
//        → exit 0 (pass 기록 있음) / 5 (없음·pending·fail)
//
...
if (!feature || feature.startsWith('--')) {
  console.error('usage: visual-check.mjs <feature> [--pass|--fail --by <name>]')
  process.exit(1)
}
...
if (has('--pass') || has('--fail')) {
  ...
  const rec = {
    feature,
    status: has('--pass') ? 'pass' : 'fail',
    checkedBy: arg('--by') ?? 'developer',
    checkedAt: new Date().toISOString(),
    note: arg('--note') ?? null,
  }
  writeFileSync(recordPath, JSON.stringify(rec, null, 2) + '\n')
  console.log(JSON.stringify({ feature, status: rec.status, checkedBy: rec.checkedBy }))
  process.exit(0)
}
