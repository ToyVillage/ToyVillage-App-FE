...
| 커밋 | `pre-commit-gate.mjs`가 승인 없는 feature 코드 커밋 거부(exit 1) |
...
| `harness/artifacts/visual-check.<f>.json` | ❌ 무시 | 육안 기록 | 아니오 |
...
- 스테이징된 파일이 어떤 feature의 `paths:`(spec frontmatter)에 속하면
- 그 feature의 `gate-check`가 통과해야만 커밋 허용, 아니면 **커밋 거부(exit 1)**
- `yarn install`(postinstall)로 자동 활성화. Claude/Codex/사람 모두 동일.
