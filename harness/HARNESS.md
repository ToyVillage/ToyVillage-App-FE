...
| **게이트는 강제, 설명 아님** | 승인 게이트는 `pre-commit` 훅으로 커밋 경계에서 실제 차단. 에이전트 선의에 의존하지 않음. |
| **기존 우선(Existing wins)** | 저장소 토큰·컴포넌트·ESLint 경계가 Figma 출력보다 우선. |
| **상태는 프로세스 죽음을 견딘다** | 교차 턴 상태(승인·루프·육안확인)는 전부 파일. |
...
├─────────────────────────────────────────────────────────────┤
│  ① 결정적 (node 스크립트 + 훅)                                 │
│     scripts/*.mjs · .githooks/pre-commit · playwright.config │
└─────────────────────────────────────────────────────────────┘
```
...
harness/
  ...
  artifacts/                 per-run 임시 산출물 (.gitignore)
    ...
    visual-check.<feature>.json  육안 확인 기록

scripts/
  map-tokens.mjs · verify.mjs · gate-check.mjs · loop-guard.mjs
  run-scenarios.mjs · approve.mjs · pre-commit-gate.mjs · install-hooks.mjs
.githooks/pre-commit         승인 게이트 강제(커밋 경계)
...
[입력] 개발자: harness/specs/<feature>.spec.md 작성
 ...
 └───────────────────────────────────────────────────────────────┘
   ...
   ⑥ 가드레일  loop-guard.mjs: N=3 OR 무진전 → STOP → 개발자 반환
        │
        ▼
   ⑦ 최종 육안 확인 (개발자)  Figma 원본 vs 로컬 비교
      · 결과 → artifacts/visual-check.<feature>.json
        │
        ▼
   [커밋]  git commit  ── ★ pre-commit 훅: paths 코드에 유효 승인 없으면 거부
```
...
| ⑦ 육안 | `visual-check.mjs`로 개발자 pass 서명 필요 |
| 커밋 | `pre-commit-gate.mjs`가 **승인 OR 육안확인 없는** feature 코드 커밋 거부(exit 1) |
...
| `visual-check.mjs` | `yarn harness:visual <f> ...` | 육안 확인 서명(`--pass`/`--fail`) / 검증 | 0 pass / 5 미확인 |
| `run-scenarios.mjs` | `yarn verify:e2e <f>` | gate-check 먼저 → Playwright 실행 | 0 pass / 3·4 게이트 / 1 실패 |
| `pre-commit-gate.mjs` | (훅 자동) | 스테이징된 feature 코드 승인 강제 | 0 통과 / 1 거부 |
| `install-hooks.mjs` | `yarn hooks:install` / postinstall | `core.hooksPath=.githooks` 설정 | 0 |
...
| `harness/approvals/<f>.visual-check.json` | ✅ 커밋 | 육안 확인 서명(pass) | **예** |
...
- 없으면 exit 3, 무효면 exit 4 → 퍼블리싱/e2e **차단**

### 7-2. pre-commit 훅 (진짜 강제점)
`.githooks/pre-commit` → `pre-commit-gate.mjs`:
- 스테이징된 파일이 어떤 feature의 `paths:`(spec frontmatter)에 속하면, 그 feature는
  **①승인 게이트(gate-check) 통과 AND ②육안 확인(visual-check) `pass` 기록**이 모두 있어야 커밋 허용.
- 하나라도 없으면 **커밋 거부(exit 1)**. `yarn install`(postinstall)로 자동 활성화. Claude/Codex/사람 모두 동일.

### 7-5. 육안 확인 서명 (stage ⑦ 강제)
`visual-check.mjs`:
- 개발자가 `yarn dev`로 Figma와 비교 후 `--pass`/`--fail` 서명 → `approvals/<f>.visual-check.json`(durable).
- 육안 확인은 사람 몫이라 자동 판정은 못 하지만, **명시적 서명이 없으면 pre-commit이 커밋을 막는다**(e2e 통과만으로는 커밋 안 됨).

### 7-3. 내용 해시 (승인-사용 불일치 방지)
...
| pre-commit 훅 | 게이트를 설명이 아니라 커밋 경계에서 실제 강제. |
...
- **육안 확인**: 사람 몫(`visual-check.*.json` pending). 자동화 안 함(의도).
