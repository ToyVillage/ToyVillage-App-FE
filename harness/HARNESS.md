...
[입력] 개발자: harness/specs/<feature>.spec.md 작성
 ...
 │  · map-tokens.mjs → 의미 토큰 후보/direct CSS 값 분리(읽기전용) │
 ...
 └───────────────────────────────────────────────────────────────┘
   ...
   ③ 퍼블리싱 (AI)   ── gate-check 먼저(없으면 STOP)
      ...
      · design-rules 준수(Emotion 의미 토큰/직접 구현값/TanStack/Zustand/Axios/Router/FSD 경계)
        │
        ▼
   ④ 자동검증 (인너 루프)  verify.mjs = yarn verify(lint→typecheck→style-policy→build)
...
|---------|-----------|------|-----------|
| `map-tokens.mjs` | `yarn harness:map-tokens <f>` | solid color/font family 후보와 px·rgba 등 direct CSS 값을 분리. **tokens.ts 읽기 전용** | 0 성공 / 1 입력 없음 |
| `check-style-policy.mjs` | `yarn style-policy` | tokens.ts의 구현값 그룹과 px/rgba 계열 값 차단 | 0 pass / 1 위반 |
| `verify.mjs` | `yarn harness:verify` | `yarn verify`(lint/typecheck/style-policy/build) 래핑, `{pass,firstError,errorHash}` | 0 pass / 1 fail |
...
yarn harness:map-tokens <feature>     # 의미 토큰 후보 + direct CSS 값 분리
...
| 의미 토큰 최소화 + 직접 구현값 분리 | solid color/font family만 명명하고 px·rgba·spacing·radius 등은 Emotion이 소유해 토큰 비대화를 막음. |
