...
## 7-0. Figma 소스는 라이브가 진실 (캐시 단정 금지)

- 논의 대상 프레임은 **그 nodeId로 `get_figma_data`를 라이브 재조회**해 확인한다. `harness/artifacts/*.figma.txt`는 per-run 캐시(HARNESS.md §6, non-durable)이므로 **기능의 존재/부재를 이 캐시로 단정하지 않는다**(stale 가능).
- 스펙(`specs/<feature>.spec.md`)의 nodeId도 stale일 수 있다. 개발자가 URL을 주면 **그 URL의 nodeId를 우선**하고, 어긋나면 스펙 nodeId를 갱신한다.
- 아이콘 등 에셋은 `download_figma_images` 전에 `find/grep`으로 repo에 같은 에셋이 있는지 확인하고, 있으면 재사용한다(공유가 필요하면 `src/shared/ui/assets`로 승격해 단일 소스화).
