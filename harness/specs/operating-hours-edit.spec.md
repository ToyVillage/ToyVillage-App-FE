...
- 시·분 입력에서 `Enter`를 누르면 `저장하기` 클릭과 동일한 검증·저장 흐름을 실행한다.
...
- 시·분 입력 중 `Enter`로 폼을 제출할 수 있고, 오전·오후 버튼에서는 `Enter`가 선택 조작으로 유지된다.
...
- S8: 시·분 입력에서 `Enter` → 유효한 영업시간을 저장하고 목록으로 이동한다.
